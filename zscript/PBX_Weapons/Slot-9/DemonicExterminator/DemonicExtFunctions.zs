extend class PBX_DemonExt{
	...
	action void UNM_FireLasers()
	{
		int lv = Player.ReFire%3*5+5;
		for(int i = -lv; i <= lv; i += lv)
			A_RailAttack(LASER_DAMAGE, 0, false,
				color1: "FF0000", color2: "FF0000",
				flags: RGF_SILENT | RGF_FULLBRIGHT | RGF_EXPLICITANGLE | RGF_NORANDOMPUFFZ,
				pufftype: "Unmaker64Puff",
				spread_xy: i,
				duration: 5,
				sparsity: 2,
				spawnclass: "UnmakerLaser64Spark",
				spawnofs_z: -1,
				spiraloffset: 270,
				limit: 1);
	}
