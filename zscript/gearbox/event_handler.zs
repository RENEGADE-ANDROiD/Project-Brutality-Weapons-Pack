  ...
  bool inputProcess(InputEvent event)
  {
  ...
  else if (mactivity.ismelee())
	{
		...
		switch (input)
		{
			...
			case InputSelectPrevWeapon: tickIf(mMeleemenu.selectPrev()); mWheelController.reset(); break;
			case InputConfirmSelection: confirmSelection(); close(); break;
			case InputClose:            close(); break;
			...
			default:
			{
				...
				int slot = gb_Input.getSlot(input);
				int index = (slot == 0) ? 9 : slot - 1;
				tickIf(mMeleemenu.setSelectedIndex(index));
				break;
			}
		}
		return true;
...
// private: ////////////////////////////////////////////////////////////////////////////////////////
  ...
  private ui void toggleMelee()
  {
	if(mActivity.ismelee()) close();
	else openMelee();
  }
  ...
  private ui void openMelee()
  {
	if (gb_Player.isDead() || mMeleemenu.noMelee())
	{
		mSounds.playNope();
		return;
	}
